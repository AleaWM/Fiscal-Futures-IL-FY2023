...
fiscal_gap2
#ggsave(plot = fiscal_gap2, filename= "Figure1.eps")
...
#| fig-cap: 
#| fig-subcap: 
#| - "Largest Expenditures for FY2023"
#| - "Largest Revenue Sources for FY2023"
...
#ggsave(plot = rev_dotplot, file = "Figure3.eps")
...
exp_long %>%
  ...
  geom_text(aes(x = ifelse(Year == "2023", (as.numeric(Dollars)+1100), NA),  
                label = ifelse(Year == "20223", Category_name, "")),  
            hjust = 0,
            size = 2.8) +
...
top3
#ggsave(plot = top3, file = "Figure5.eps")
```
...
## DELETE Change from Previous Year
...
CAGR_expenditures_summary_tot <- move_to_last(CAGR_expenditures_summary_tot, 29) # Update year in the filter() and summarize() commands to current year.
cagr_10 <- calc_cagr(exp_long, 10) %>% 
  filter(Year == 2023) %>%
...
```{r }
#| label: tbl-yearlychange-table-revenue
#| tbl-cap: Yearly Change in Revenue Sources
...
expenditure_change2 %>% 
  kbl(caption = "Yearly Change in Expenditures - All Categories", row.names = FALSE) %>% 
  kable_classic() %>%
    row_spec(31, bold = T, color = "black", background = "gray")
```

```{r summary-file-allcategories, eval=FALSE}
library(openxlsx)
...
write.xlsx(dataset_names, file = 'summary_file_FY2023_allcats.xlsx')
```
...
Creating the excel file for all categories with summary tables plus wide and long formats of the aggregated data:
...
write.xlsx(dataset_names, file = 'summary_file_FY2022_allcats_droppedStateCURE.xlsx')
```
...
```{r}
#| label: tbl-yearly-change
#| fig-cap: 
#| fig-subcap: 
#| - "Revenue Yearly Change"
#| - "Expenditure Yearly Change"
...
## Largest Categories Summary Tables
...
```{r summary-file-droppedCURE, eval=FALSE}
#install.packages("openxlsx")
library(openxlsx)
...
write.xlsx(dataset_names, file = 'summary_file_FY22_MajorCats_WithTotals_droppedCURE.xlsx')
```
