...
#write_csv(allrevfiles23, "allrevfiles.csv")
...
# filling in missing values for source_name
allrevfiles23 <- allrevfiles23 %>% 
  mutate(source_name = 
           case_when(
             fy == "2018" ~ "rev IOC 2018",
             fy =="2021" ~ "rev IOC Nov 2022",
             fy == "2022" ~ " rev IOC Nov 2023",
             TRUE ~ source_name)) %>%
  mutate(from_fund = ifelse(from_fund==".", NA, from_fund) )

allexpfiles23 <- allexpfiles23 %>% 
  mutate(source_name = 
           case_when(
             fy == "2018" ~ "exp IOC 2018", # was missing label
             fy == "2021" ~ "exp IOC Nov 2022", # when FOIA data request was received
             fy == "2022" ~ "exp IOC Nov 2023",
             TRUE ~ source_name))

#write_csv(allexpfiles23, "allexpfiles.csv")
...
Comparing all known agencies and funds from past years to the most recent year of data tells us if any new funds, agencies, sources, etc. began to be used or if an old fund number was reused for a new function. Tracking changes over time is a crucial step in the Fiscal Futures project methodology.
...
3.  Update fund_ab_in_2023.xlsx with any changes.
...
New funds were manually added to the funds_ab_in.xlsx file and recoded funds were addressed in the code chunks below and updated in the same excel file that helps track changes over time.
...
funds_ab_in_2023 = readxl::read_excel("funds_ab_in_2023.xlsx")
...
-   the initial combined years of data are saved as dataframes named `exp_1998_202` and `rev_1998_2022`. These are then saved as exp_temp and rev_temp while recoding variables. This is BEFORE category groups are created and cleaned below.
...
write.csv(exp_temp, "allexp_fy22_recoded_April262023.csv")
write.csv(rev_temp, "allrev_fy22_recoded_April262023.csv")
